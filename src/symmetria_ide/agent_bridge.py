...
class AgentBridgeClient(QObject):
    ...
    # (agent-ownership inversion, Phase 4) The bridge-routed `inject_requested`
    # signal + `send_inject_result` reply were removed — STT injection is now a
    # direct shell→IDE round-trip on the IDE's own agent socket (agent_events.py).
    # This client is publish + subscribe only (subscribe stays for opencode
    # activity snapshots); it no longer receives commands from the hub.
            ...
            try:
                ...
                for line in reader:
                    ...
                    try:
                        ...
                        continue
                    # The subscription feed is untyped snapshot lines. Since the
                    # inversion (Phase 4) the hub no longer routes inject commands
                    # back to us, so every inbound line is a snapshot.
...
def emit_gc_safe(signal: SignalInstance, *payload) -> None:
    ...
    by the bridge reader thread, AppController's one-shot workers, and
    the WorktreeWatcher's no-payload `changed` emit (hence *payload).
    """
