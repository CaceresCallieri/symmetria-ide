...
- **⚠ `runJavaScript`-based tools (`eval_js`/`snapshot`/`click`/`fill`) are UNRELIABLE under `QT_QPA_PLATFORM=offscreen` — verify them LIVE, not headless.** `runJavaScript` only invokes its callback once the page commits a frame; the offscreen platform has no continuous frame driver, so the callback stalls non-deterministically (reproducible even on the simplest direct `BrowserAutomation.eval_js` path — it is NOT an MCP/bridge bug). `navigate`/`list_windows`/`open` don't use `runJavaScript` (navigate sets `view.url` directly) and ARE reliable headless. A live Wayland compositor drives frames continuously, so all tools work on real hardware. Do not chase the headless `eval_js` hang as a code bug — it's a platform limitation. Live check: spawn an agent in the IDE, have it `browser_open` + `browser_eval_js`/`browser_snapshot`.
