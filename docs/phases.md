...
**Stage 6 — agent-owned topology: no tab, attention badge, orphan cleanup (SHIPPED 2026-06-19):**

- The browser stopped being a first-class central surface and became **agent-owned**. The standalone **switcher tab was removed**; `Ctrl+Shift+B` was repurposed from a surface toggle into `jump_to_focused_agent_browser` (the keyboard twin of clicking a chip's globe — `toggle_browser_terminal`/`swap_to_browser` deleted). You reach the browser ONLY through its owning agent: the chip globe (mouse) or `Ctrl+Shift+B` (keyboard). The chord is **agent-only** (focused agent owns nothing → no-op; already on browser → back to the surface you came from, per the 2026-06-21 swap-last-two change — was hard-coded terminal).
- **Notify, don't yank:** `_open_browser_for_mcp` opens with `focus=False`, so an agent opening a window lights the chip globe but no longer switches the user's surface. Manual `Ctrl+T` still focuses.
- **Attention badge:** a third server tool `browser_request_attention(message="")` → `_set_browser_attention_for_mcp` lights a notification dot on the agent's globe (`agentBrowserAttention` → QML `attentionDot`); requires an owned window; cleared on view / last-window-close / agent death. The `message` is stored for a future tooltip / freedesktop notification.
- **Orphan cleanup (leak fix):** `close_agent` → `_release_agent_browser_windows` closes the windows the agent solely owns (co-owned-by-a-living-agent kept) before forgetting links — once the tab is gone, a dead agent's window would otherwise both leak RAM AND be unreachable. The `npx chrome-devtools-mcp` process is a child of the claude CLI, so it dies with the agent.
- **Authoritative record: CLAUDE.md "The browser panes" Stage 6.**

**Checkpoint (Stage 6) — MET:** unit tests pin notify-not-yank, attention set/reject/clear, the agent-only jump chord, and solely-owned-vs-co-owned window cleanup on agent close (`tests/test_app_controller_browser_pool.py`); offscreen engine-load renders the chip delegate (globe + attention dot) without binding errors. Live verification (interactive globe click + agent-driven `browser_request_attention` + RAM/`pgrep` check on agent close) pending a Hyprland session.
