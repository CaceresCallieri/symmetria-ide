...
2. **Agent-frontend ecosystem to lean on.** Mature React Claude-Code UIs exist to study/harvest (e.g. `siteboon/claudecodeui` — React 18 + TS + Vite + Tailwind; multi-agent: Claude/**OpenCode**/Cursor/Codex/Gemini). **Verified 2026-06-05** (correcting an earlier wrong note): it is **SDK-based, not CLI-scraping** — it drives Claude via `@anthropic-ai/claude-agent-sdk` + `canUseTool`, the same architecture we chose (so adopting its UI does NOT regress our permission model — its approval UI is transport-decoupled and maps onto our sidecar ~1:1). **OpenCode is already a first-class provider.** Verdict: **harvest patterns, do NOT fork** — it is **AGPL-3.0-or-later** with permanently-mixed GPL-3.0 third-party copyright and no CLA/relicensing path, so forking it into a *distributed or hosted* Symmetria IDE risks viral copyleft over the whole app (fine only for undistributed personal use). See §7 / the agent-pane reference note for what to lift. Multi-agent (OpenCode, Pi, Codex, …) is the web ecosystem's strong suit.
...
2. **Agent panel + logic** (Claude Code frontend; keep our SDK sidecar's `canUseTool` permission model — do not regress to CLI-based permission gating). Includes the embedded browser pane (§2.3). **Goal: an agent-AGNOSTIC pane — Claude first, then OpenCode, later Pi/Codex/Antigravity.** Reference to harvest (NOT fork — AGPL, see §2): `siteboon/claudecodeui`. Its genuinely valuable, reusable artifact is the **`NormalizedMessage` seam** — every agent implements `normalizeMessage(raw) → NormalizedMessage[]`, collapsing all agents into one frontend-rendered shape; its **OpenCode parser is a ready blueprint** for our Phase-2 adapter. Also worth lifting as *patterns* (reimplement, don't copy AGPL verbatim into a shipped build): its tool-call rendering system (`src/components/chat/tools/` — drill-in, diff viewer, plan display — the "tool-call drill-in" CLAUDE.md lists as deferred) and its transport-decoupled permission-panel registry. **Improve on its one design flaw:** it normalizes *messages* cleanly but special-cases *execution* in per-agent if-ladders with hardcoded provider-id lists. We unify BOTH metadata and execution behind a single provider interface (one `run-a-turn` method per agent, registry as the single source of truth) — so Claude and OpenCode are two implementations of one interface, and each later agent is just one more.
