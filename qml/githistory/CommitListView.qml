...
FocusScope {
    ...
    // Fixed delegate row height — shared by the delegate and the nav's
    // Ctrl+D/U half-page math so the two can't drift out of sync. Roomier
    // than a terminal row so the inset clay selection pill breathes.
