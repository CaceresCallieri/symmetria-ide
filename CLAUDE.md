...
| `permission_mode_changed` | sidecar-synthesized; `{mode}`        | NOT routed by `SessionModel`; `AppController._on_session_event` mirrors into `_permission_mode` for the QML pill |
...
Permission mode flow: the agent pane's chrome row carries a Theme-tokened pill bound to `controller.permissionMode`. Shift+Tab on the pane (root or composer) cycles `default → acceptEdits → bypassPermissions → plan → default` via `AppController.cycle_permission_mode()`, which dispatches `set_permission_mode` to the sidecar. The sidecar awaits `Query.setPermissionMode(mode)`; on success it emits a `permission_mode_changed` echo that `AppController._on_session_event` mirrors into `_permission_mode` (the cycle slot does NOT optimistically mutate — the SDK can reject a transition, e.g. into `bypassPermissions` if `allowDangerouslySkipPermissions` is unset; we set that flag eagerly in `Options` so transitions succeed, but the echo remains the source of truth). The sidecar's `canUseTool` callback short-circuits per mode: `bypassPermissions` auto-allows everything (echoes `input` as `updatedInput` per gotcha #24), `plan` auto-denies everything, `acceptEdits` auto-allows the four edit tools (`Edit`/`Write`/`MultiEdit`/`NotebookEdit`), `default` (and `acceptEdits` for non-edit tools like `Bash`/`Read`/`Grep`) round-trips through the in-pane card. Auto-resolved cases do NOT emit a `permission_request` envelope, so the QML card is correctly suppressed for the modes that bypass it. `_on_session_closed` resets `_permission_mode` to `default` so the pill doesn't lie about a dead subprocess's last mode.
