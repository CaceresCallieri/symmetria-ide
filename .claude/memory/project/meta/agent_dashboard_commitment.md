...
description: Cross-project commitment — IDE will dual-feed (hooks + SDK) the existing Symmetria Shell agent dashboard; orchestrator.nvim retires once IDE meets gates. Phased execution plan lives in docs/orchestrator-replacement-prd.md.
type: project
...
**How to apply:** Read `docs/agent-dashboard-integration.md` for the architectural commitment (why hooks + SDK feeds coexist, what the dashboard depends on). Read `docs/orchestrator-replacement-prd.md` for the phased execution plan (Phases A–G: per-instance refactor → multi-instance keybinds → spawn semantics → SDK→bridge emitter → spawn menu → orchestrator deprecation → edit tracker). Both docs are the canonical references; the PRD names files, line numbers, acceptance criteria, and open questions per phase. Cross-references to the Symmetria Shell side (`AgentService.qml`, `agent-bridge.py`, `symmetria-agent-hook.py`) are listed in both. Do not duplicate either doc into memory — they live in `docs/` and `CLAUDE.md` § "Where to look first" links to both.
