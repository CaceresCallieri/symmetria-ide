...
| shared toolkit palette (`ui_scheme`) | `SYMMETRIA_UI_SCHEME` (**deleted**, in the `XDG_CONFIG_HOME` fixture). ⚠ It outranks `XDG_CONFIG_HOME`, so redirecting the XDG root alone does not isolate it |
...
| OpenCode history reader (spawns the developer's real `opencode` against their live session store, ~1.56 s per call) | `SYMMETRIA_IDE_OPENCODE_BIN` (set to a throwaway path — a name that cannot exist is enough; the reader logs the `FileNotFoundError` and returns no rows, which is why the inert value is a bad PATH rather than a deletion). Gates nothing else: the reader resolves it per call, so it contains even a scan started without the thread-index gate. `tests/unit/test_agent_threads_opencode.py::test_opencode_reader_defaults_to_opencode_executable` deletes it and asserts the production default, per the follow-up rule below |
| agent thread index (reads the developer's real `~/.claude/projects` — 315 transcripts) | `SYMMETRIA_IDE_THREAD_INDEX=0` (set — `0` IS the switch) AND `SYMMETRIA_IDE_CLAUDE_PROJECTS_DIR` (throwaway dir). ⚠ TWO vars because they contain different reaches: the first gates `AppController._request_thread_index`, the second redirects `ClaudeThreadReader`'s default root for any path that reaches a reader without the gate. The gate is deliberately NOT inside `AgentThreadIndexer.start`, so `tests/integration/test_agent_thread_indexer.py` can drive a live worker with fake readers |
