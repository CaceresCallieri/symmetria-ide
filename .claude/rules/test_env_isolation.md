...
| subscription-usage poller (real HTTPS to the user's Anthropic account + a real `codex app-server` spawn) | `SYMMETRIA_IDE_USAGE_POLL=0` (set, not deleted — `0` IS the switch) |
...
- Neutralise the env in an **autouse, function-scoped** fixture in `tests/conftest.py`, following the existing `_isolate_*` fixtures there. Point paths at a throwaway dir; delete flags rather than setting them to `0` (a test that wants the behaviour opts in with its own `setenv`, last-writer-wins).
